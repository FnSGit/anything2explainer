...
| 公式 | **MathJax SVG**（`src/formula.tsx` 的 `MathFormula`） | 用 `fontSize` 控尺寸：**嵌套分式墨迹高 ≈2.59×、单层 ≈1.2×**；换算表见 `math-physics.md` §2 |
...
| `TechSub`（`ui.tsx`） | 中英配对的副标：主体下面那行另一种语言，灰色小字（HUD 22px / 章节卡 26px），**不用紫色、不与主体同大小**——紫色只给当前重点，等大同色会读成两个主体 |
...
- **图表卡**：440×300 白边黑底，网格 #4A4A4A 1px，**坐标字 ≥22px**（与下条「图内说明最小 22px」一致——旧版误写 15–18px），曲线 2–3px。
- **坐标系图卡**：`Axes`（w566×h372 是常用档）。**h≥372 才能自身当主角**（实测主体尺度 481）；刻度字 ≥22px；网格 #4A4A4A 1px；曲线白 2–3px。紫色预算见 `math-physics.md` §3 规则 2。
- **公式**：用 `MathFormula`（真 LaTeX 预生成 SVG，离线、确定性、不需要 LaTeX 与字体文件），**不要手摆字体**；揭示动画用 `MathFormulaReveal`；灰小字注参数（k=60）。尺寸换算见 `math-physics.md` §2。
